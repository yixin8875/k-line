...
@interface GLSystrayAppDelegate: NSObject <NSApplicationDelegate>
  - (void) add_or_update_menu_item:(MenuItem*) item;
  - (IBAction)menuHandler:(id)sender;
  @property (assign) IBOutlet NSWindow *window;
  @end

  @implementation GLSystrayAppDelegate
{
...
static GLSystrayAppDelegate *gSystrayDelegate;

void registerSystray(void) {
  NSNotification *launchNotification = [NSNotification notificationWithName:NSApplicationDidFinishLaunchingNotification object:NSApp];
  if ([NSThread isMainThread]) {
    gSystrayDelegate = [[GLSystrayAppDelegate alloc] init];
    [gSystrayDelegate applicationDidFinishLaunching:launchNotification];
    return;
  }
...
int nativeLoop(void) {
  // Wails owns the main application loop on macOS.
  return EXIT_SUCCESS;
}

void runInMainThread(SEL method, id object) {
  if (gSystrayDelegate == nil) {
    return;
  }
  [gSystrayDelegate performSelectorOnMainThread:method
                                     withObject:object
                                  waitUntilDone:YES];
}
